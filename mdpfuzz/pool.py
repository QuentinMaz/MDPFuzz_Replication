...
class Pool(ABC):
    ...
    def select(self, rng: np.random.Generator) -> Tuple[np.ndarray, float]:
        ...
        if np.sum(self.sensitivities) == 0:
            ...
            index = rng.choice(
                len(self.inputs), p=(self.sensitivities / np.sum(self.sensitivities))
            )
...
class IndexedPool(Pool):
    ...
    def load(self, path: str, as_integers: bool = False):
        ...
        if self.is_integer:
            ...
            self.inputs = [
                i for i in np.loadtxt(path + "_inputs.txt", delimiter=",", dtype=int)
            ]
            self.crashes = [
                i for i in np.loadtxt(path + "_crashes.txt", delimiter=",", dtype=int)
            ]
        else:
            self.inputs = [
                i for i in np.loadtxt(path + "_inputs.txt", delimiter=",", dtype=float)
            ]
            self.crashes = [
                i for i in np.loadtxt(path + "_crashes.txt", delimiter=",", dtype=float)
            ]

        self.indices = [self._key(i) for i in self.inputs]
        self.added = [
            i for i in np.loadtxt(path + "_added.txt", delimiter=",", dtype=int)
        ]
        self.selected = [
            i for i in np.loadtxt(path + "_selected.txt", delimiter=",", dtype=int)
        ]
        self.oracles = [
            i for i in np.loadtxt(path + "_oracles.txt", delimiter=",", dtype=int)
        ]
        self.rewards = [
            i for i in np.loadtxt(path + "_rewards.txt", delimiter=",", dtype=float)
        ]
        self.sensitivities = [
            i
            for i in np.loadtxt(path + "_sensitivities.txt", delimiter=",", dtype=float)
        ]
        self.coverages = [
            i for i in np.loadtxt(path + "_coverages.txt", delimiter=",", dtype=float)
        ]
...
class LightPool(Pool):
    ...
    def save(self, path: str):
        if len(self.inputs) == 0:
            ...
            # saves inputs and crashes
            is_integer = np.issubdtype(self.inputs[0].dtype, np.integer)
            if is_integer:
                np.savetxt(
                    path + "_inputs.txt", self.inputs, fmt="%1.0f", delimiter=","
                )
                np.savetxt(
                    path + "_crashes.txt", self.crashes, fmt="%1.0f", delimiter=","
                )
            else:
                np.savetxt(path + "_inputs.txt", self.inputs, delimiter=",")
                np.savetxt(path + "_crashes.txt", self.crashes, delimiter=",")
            np.savetxt(
                path + "_selected.txt", self.selected, fmt="%1.0f", delimiter=","
            )
            # np.savetxt(path + '_rewards.txt', self.rewards, delimiter=',')
            # np.savetxt(path + '_sensitivities.txt', self.sensitivities, delimiter=',')
